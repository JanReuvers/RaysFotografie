...
categoryGrid.innerHTML = Object.entries(galleries).map(([key,g],i)=>`<button class="category-card" data-gallery="${key}"><img src="${A(g.cover)}" alt="${g.title}" loading="lazy"><div><span>0${i+1} — serie</span><h3>${g.title}</h3></div></button>`).join('');
...
function showGallery(key){const g=galleries[key];activePhotos=g.photos;document.querySelector('#gallery-title').textContent=g.title;photoGrid.innerHTML=g.photos.map((p,i)=>`<button data-index="${i}" aria-label="Open foto ${i+1}"><img src="${A(p)}" alt="${g.title} fotografie ${i+1}" loading="lazy"></button>`).join('');galleryView.hidden=false;galleryView.scrollIntoView({behavior:'smooth'});history.replaceState(null,'',`#${key}`)}
